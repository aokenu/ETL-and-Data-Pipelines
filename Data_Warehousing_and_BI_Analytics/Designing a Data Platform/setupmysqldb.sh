...
mysql --host=172.17.0.2 --port=3306 --user=root --password=root -e "create database sales;
...
mysql --local-infile=1 --host=172.17.0.2 --port=3306 --user=root --password=root -e "use sales;load data infile '/var/tmp/sales_olddata.csv' into table sales_data fields terminated BY ','lines terminated BY '\n';"

# Loading data from sales_newdata.csv into sales_data table.This csv consists of data updated with the current timestamp.
mysql --local-infile=1 --host=172.17.0.2 --port=3306 --user=root --password=root -e "use sales;load data infile '/var/tmp/sales_newdata.csv' into table sales_data fields terminated BY ',' lines terminated BY '\n'(rowid,product_id,customer_id,price, quantity);"
